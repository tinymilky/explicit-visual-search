...
def visualize_one(
    ...
    for i, bb in enumerate(bboxes, start=1):
        ...
        is_sem = bool(bb.get("is_semantic", False))

        sem_tag = "semantic" if is_sem else "extractive"
        legend = f"{i}) {role} | {label} | {sem_tag}"
...
def main():
    ...
    with open(args.input, "r", encoding="utf-8") as fin:
        for idx, line in enumerate(tqdm(fin, total=total, desc="Visualizing", dynamic_ncols=True), start=1):
            ...
            answer = row.get("possible_answers", "")
            answer = answer[0] if len(answer) == 1 else " | ".join(answer)
...
python viz_gemini_bboxes.py --input ./tmp.jsonl --image-root ./valid_images --output-dir ./viz_out_tmp
